...
| `figma/components.json` | Figma node IDs, templates, variants, brand assets, Core email templates (`email_templates`), Infinite email templates (`email_templates_infinite`) | Figma production tasks (paid media, in-app, email — Core AND Infinite) |
...
10. **Email components live in `email_templates`** — for any email task, read from `figma/components.json` → `email_templates`. Library was rebuilt 2026-05-08: 1 chassis (sample assembled email reference) + 8 heroes + 12 blocks + cta_inline + cta_block + illustrations (8 use cases) + icons (82 decorative variants) + menus (2 variants) + footers (3 languages). Compose fresh emails by stacking heroes/blocks/footer in a new auto-layout frame (1080 wide, white bg, padding 62, gap 72). Email channel uses scoped tokens (`gunmetal #1c2b29`, `teal-2025 #00dbbf`, `silver #bdbfb8`, DM Sans Bold for CTA) — see `BRAND.md § 6.1` and `tokens/brand.json` → `email_overrides`. Don't apply paid-media `teal500 #42DECA` to email.
11. **Infinite email components live in a SEPARATE library** — for any email task explicitly tagged Infinite (Product=Infinite, campaign=Infinite GTM AR, `subscription_tier=infinite`, or copy anchoring on "La membresía que se paga sola"), read from `figma/components.json` → `email_templates_infinite` (section `1165:20363`). The Infinite library was added 2026-05-14: 6 chassis reference examples (Welcome / CBU USD / SI Global Card, Light + Dark) + Menu + Hero set (2 variants) + Block set (7 variants) + CTA Button + Inline Link + Card/Info/Billing + Benefit Row + Benefit Row Numbered + Icons in 3 sizes × 2 themes + Footer + Media + Infinite badges/lockup samples. **Hard firewall: never mix `email_templates_infinite.*` components with `email_templates.*` components in the same email.** A single email is fully Infinite or fully Core. Infinite uses different tokens (cream `#f5f7f2` light bg, dark `#0f1514` dark bg, solid dark/light pill CTAs — NEVER the Core teal `--teal-2025 #00dbbf`) and different typography accents (Matter Bold for CTA, NOT DM Sans). When the brief is NOT explicitly tagged Infinite (default for everything — Core PIX, Core FX, Core Freelance, Core lifecycle), use the Core `email_templates` library.
12. **Single source of truth for shared agents** — when updating any rule that applies across channels (image library, brand voice, FSA1399 trigger, forbidden words, etc.), edit `AGENTS-SHARED.md`. The channel-specific files (`PAID-MEDIA-AGENTS.md`, `INAPP-AGENTS.md`, `EMAIL-AGENTS.md`) only own behavior that is genuinely channel-specific.
